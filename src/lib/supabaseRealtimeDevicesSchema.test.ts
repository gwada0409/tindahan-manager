...
const repair=readFileSync(resolve(process.cwd(),'supabase/migrations/202608030003_phase20_sync_device_repair.sql'),'utf8');
...
describe('Phase 16 realtime and device management contract',()=>{
 it('adds rerunnable owner-only revocation without permitting unrevocation',()=>{
  for(const sql of [migration,repair]){
   expect(sql).toContain("private.has_store_role(p_store_id, array['owner']");
   expect(sql).not.toContain("public.has_store_role(p_store_id, array['owner']");
   expect(sql).toContain("policyname = 'devices_select_store_owner'");
   expect(sql).toContain('revoked_at is null');
   expect(sql).toContain('revoke all on function public.revoke_store_device');
   expect(sql).toContain('grant execute on function public.revoke_store_device');
