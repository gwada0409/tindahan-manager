...
For download mode, the user explicitly authorizes replacement of the local working set after backup. For create/merge modes, original IDs are retained when valid UUIDs; supported legacy IDs and dependent references are converted safely. A resumed upload acknowledges a same-ID, same-version master record when its business fields already match the cloud copy, preventing stale receipt metadata from causing a repeated version conflict; genuinely different records remain queued for conflict handling. Counts and selected financial totals must validate before completion. See [the migration guide](docs/migration-guide.md).
