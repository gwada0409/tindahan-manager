...
Phase 22 repairs cross-device writes created after account linking: authenticated repository writes use the selected cloud store even when local store settings are absent, stranded per-device queue entries are adopted safely, every authenticated push repairs legacy `default` product-category references even during an incomplete migration, dependent inventory and sale retries run only after master data and inventory prerequisites, acknowledged inventory operations no longer create false pull conflicts, and interrupted same-version uploads are reconciled without overwriting different cloud data.
...
For download mode, the user explicitly authorizes replacement of the local working set after backup. For create/merge modes, original IDs are retained when valid UUIDs; supported legacy IDs and dependent references are converted safely. A resumed upload acknowledges a same-ID, same-version master record when its business fields already match the cloud copy, preventing stale receipt metadata from causing a repeated version conflict; genuinely different records remain queued for conflict handling. Baseline counts and selected numeric totals must not shrink before completion; additive cloud pulls or local writes made during a paused migration are accepted. See [the migration guide](docs/migration-guide.md).
